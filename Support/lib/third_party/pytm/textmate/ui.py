...
import subprocess
import pytm
from tm_helpers import current_word
...
def complete(choices, options = {}):
    ...
    if "initial_filter" in options and options['initial_filter']:
        command.append("--alreadyTyped %s" % sh_escape(options["initial_filter"]))
    if "static_prefix" in options and options['static_prefix']:
        command.append("--staticPrefix %s" % sh_escape(options["static_prefix"]))
    if "extra_chars" in options and options['extra_chars']:
        command.append("--additionalWordCharacters %s" % sh_escape(options['extra_chars']))
